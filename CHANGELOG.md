...
- `<DateRail>`: `showMonth` / `showWeekday` props (both default `true`) to hide the month and/or weekday line in each cell — e.g. `"Jul 28"` without the weekday, `"Tue 28"` without the month, or a compact day-number-only rail
...
- README: wrapped long code-example lines that caused horizontal scrollbars on the npm package page
- README: added live-demo and StackBlitz playground links
